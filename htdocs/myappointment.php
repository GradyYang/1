			...
			<div class="login-pad">
				...
				<?php
					...
					$patient_id = $_SESSION['id'];
							$sql = "SELECT * from request where patient_id = '$patient_id' ";
							$result = mysqli_query($conn,$sql);
							?>
							<form action="myappointment2.php" method="post">
							<select name="request" size="10" required = 'required'><?php
					...
					$sql2 = "SELECT * FROM doctor WHERE id = '$doctor_id' ";
							...
							$row2 = mysqli_fetch_array($result2);
								...
								<!--<input type="hidden" name="id" value=<?=$row['id']?>><?php echo $row['date_prefer'] . '  ' .$row2['name'] ; ?> <input type="submit" name="submit" value="Change"  /><input type="submit" name="submit" value="Delete"  /> -->
								<option value="<?=$row['id']?>"> <?php echo $row['date_prefer'] . ' - Dr.' .$row2['name'] ; ?></option>
